...
crop_dict = {
    ...
    'hay-alfalfa':{
        'file_path_state': 'hay alfalfa - price recieved dollars per ton - state.csv',
        'file_path_national': 'hay alfalfa - price recieved dollars per ton - national.csv',
        'units': 'dollar-per-ton'
        },
    'hay-excluding-alfalfa':{
        'file_path_state': 'hay excluding alfalfa - price recieved dollars per ton - state.csv',
        'file_path_national': 'hay excluding alfalfa - price recieved dollars per ton - national.csv',
        'units': 'dollar-per-ton'
        },
    'rye':{
        'file_path_state': 'rye - price recieved dollars per bushel - state.csv',
        'file_path_national': 'rye - price recieved dollars per bushel - national.csv',
        'units': 'dollar-per-bushel'
        },
    'soybean':{
        'file_path_state': 'soybean - price recieved dollars per bushel - state.csv',
        'file_path_national': 'soybean - price recieved dollars per bushel - national.csv',
        'units': 'dollar-per-bushel'
        },
    'winter-wheat':{
...
# Set the inputs
file_path_state = crop_dict[crop_use]['file_path_state']
file_path_national = crop_dict[crop_use]['file_path_national']
...
# Check if the index contains 'Other States' and drop the row
if 'Other States' in df_states.index:
    df_states = df_states.drop(index='Other States')
...
#%% Save as CSV
filepath_out = f'../_RuFaS Input Files/crops_{crop_use}-price-received_{crop_dict[crop_use]["units"]}.csv'
