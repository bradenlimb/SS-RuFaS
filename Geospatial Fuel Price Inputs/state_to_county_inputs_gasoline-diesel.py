...
## TODO Select fuel Type
data_type = 'Diesel' # Gasoline, Diesel

# Set the inputs
file_path = f'{data_type}.xls'
sheet_name = 'Use'

# Load the data into a DataFrame
df_fuel = pd.read_excel(file_path, 
                      sheet_name=sheet_name,
                      # dtype={"Date": str}
                      )
...
years_use = df_fuel.index.tolist()

# Combine NG data data with df_states
for year in years_use:
    for state in states_list:
        conversion = 1  #  gallon per gallon fuel
        if state in df_fuel.columns:
            df_states.loc[state, year] = round(df_fuel.loc[year,state] * conversion,6)
            
        else:
            state_PADD = df_states.loc[state,'PADD']
            df_states.loc[state, year] = round(df_fuel.loc[year,state_PADD] * conversion,6)
...
#%% Save as CSV
filepath_out = f'../_RuFaS Input Files/{data_type.lower()}_retail_dollar-per-gallon.csv'
