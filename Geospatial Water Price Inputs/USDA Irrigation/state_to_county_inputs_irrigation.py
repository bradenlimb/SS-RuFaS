...
conversion = 1
errors = []
years_use = [2013,2018,2023]

for year_use in years_use:
# Load the Water data for US average
    ...
    df_states.loc['US',year_use] = df_water.loc['US TOTAL','Value'] * conversion
    ...
    for state in states_list:
        try:
            df_states.loc[state, year_use] = df_water.loc[state.upper(),'Value'] * conversion
        except:
            errors.append(state)
            df_states.loc[state, year_use] = df_states.loc['US',year_use]
...
#%% Save as CSV
filepath_out = f'../../_RuFaS Input Files/water_irrigation_dollar-per-acre-foot.csv'
